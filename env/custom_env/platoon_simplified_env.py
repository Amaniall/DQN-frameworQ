...
class PlatoonEnvSimp:
    ...
    def __init__(self, gui=False, log=True, rnd=(False, False)):
        ...
        self.sub = {}
        self.join_list = []
        self.communicating_vehicles = set()
        ...
        self.tlp_list = []
        self.obs_done = {}
        self.edges = ["gneE2", "gneE3", "gneE4", "gneE5", "gneE6"]
        ...
        self.edge_m = ["gneE2", "gneE3", "gneE4", "gneE5", "gneE6"]
        self.max_try = 5
        self.n_failure = 0
        self.n_success = 0
        self.t_reward = 0
    ...
    def simulation_step(self):
        try:
            ...
            if len(self.join_list) > 1:
                ...
                if self.is_vehicle_known(joiner):
    ...
    def remove_from_communications(self, vehicle_id):
        if hasattr(self, 'communicating_vehicles') and vehicle_id in self.communicating_vehicles:
            self.communicating_vehicles.remove(vehicle_id)
            self.topology.pop(vehicle_id, None)
            # Further actions to handle removal from communication systems, if any
    ...
    def step(self, action):
        if len(self.join_list) > 1:
            ...
            if min(o[-4], o[-5]) < 0 and action == 5:
                self.danger_try += 1

            for i in range(5):
                if action == 5:
                    if self.done() == 0:
                        traci.vehicle.setSpeed(self.join_list[1], self.o0[0])
                    elif self.join_list and self.join_list[1] in traci.vehicle.getIDList():
                        traci.vehicle.setSpeed(self.join_list[1], -1)
                self.simulation_step()
                print (self.s_step)
                try:
            ...
            self.reward_info = r
            self.n_step += 1
            if d and self.join_list and self.join_list[1] in traci.vehicle.getIDList():
                traci.vehicle.setSpeed(self.join_list[1], -1)
            if r == -100:
                self.n_collison += 1
        ...
        if len(self.join_list) > 1:
            ...
            if joiner_id in self.tlp_list or joiner_id not in traci.vehicle.getIDList():
                ...
                if do == 1:
                    ...
                    if self.get_veh_laneIdx(joiner_id) != self.get_veh_laneIdx(joiner_id) and self.action:
                        r -= 10
    ...
    def get_platoons_available(self, edge, veh):
        ...
        for leader in set(self.platoons.keys()).intersection(veh_edge_list):
            ...
            if self.platoons[leader]["state"] == 0 and leader not in [l[0] for l in self.join_list]:
    ...
    def get_join_list(self):
        ...
        if not self.multi_joiners:
            ...
            j_list = copy.deepcopy(self.join_list)
            e_occupied = [traci.vehicle.getRoadID(j_l[1]) for j_l in j_list]
            for e in self.edge_m:
                if e not in e_occupied:
                    ...
                    if cavs:
                        for joiner in cavs:
                            if not j_list or (joiner not in [list[1] for list in j_list]):
                                p_list = self.get_platoons_available(e, joiner)
                                if len(p_list) > 0:
                                    '''p, s = self.get_score_4_veh(e, joiner, p_list)
                                    position = self.choose_position(joiner, p)
    ...
    def cav_penetration_rate(self):
        self.cav_p_rate = random.randint(2, 7) / 10 + 0.3
