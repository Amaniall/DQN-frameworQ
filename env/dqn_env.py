...
from plexe.plexe import Plexe, ACC, CACC, DRIVER, FAKED_CACC, SPEED, POS_X, POS_Y
...
logging.basicConfig(filename='vehicle_debug.log', level=logging.INFO)

class DqnEnv:
    SUMO_ENV = "./env/custom_env/"

    def min_max_scale(self,log,x, feature):
        return (x - self.min_max[feature][0]) / (self.min_max[feature][1] - self.min_max[feature][0])

    def __init__(self,m, p=None, gui=False, log=False, rnd=(False, False)):
        ...
        self.sub = None
        self.cav_p_rate = 0.5
        self.min_platoon_size=1
        self.max_platoon_size=20
    ...
    def simulation_step(self):
        ...
        # Check for collisions and remove the involved vehicles
        ...
        if collision_vehicles:
            for vehicle_id in collision_vehicles:
                if vehicle_id in traci.vehicle.getIDList():
                    ...
                    logging.warning(f"Vehicle {vehicle_id} collided and was removed from the simulation")
    ...
    """def step(self, action):
        for vehicle_id in traci.vehicle.getIDList():
            if vehicle_id not in traci.vehicle.getIDList():
                continue  # This line is redundant and will never be executed
            else:
                traci.vehicle.setSpeed(vehicle_id, action)
        traci.simulationStep()

        # Handle collisions and debug vehicle IDs if necessary
        # self.handle_collisions()
        # self.debug_vehicle_ids()
    ...
    def communicate(self):
        ...
        for vid, l in list(self.topology.items()):  # Using list() to avoid RuntimeError if modifying dict during iteration
            if "leader" in l.keys():
                leader_id = l["leader"]
                if leader_id in traci.vehicle.getIDList():
                    # get data about platoon leader
                    ld = self.plexe.get_vehicle_data(leader_id)
                    # pass leader vehicle data to CACC
                    self.plexe.set_leader_vehicle_data(vid, ld)
                else:
                    logging.warning(f"Leader vehicle {leader_id} not found in simulation, removing from topology")
                    del self.topology[vid]  # Remove the reference

            if "front" in l.keys():
                front_id = l["front"]
                if front_id in traci.vehicle.getIDList():
                    # get data about front vehicle
                    fd = self.plexe.get_vehicle_data(front_id)
                    # pass front vehicle data to CACC
                    self.plexe.set_front_vehicle_data(vid, fd)
                else:
                    logging.warning(f"Front vehicle {front_id} not found in simulation, removing from topology")
                    del self.topology[vid]  # Remove the reference
    # https://github.com/michele-segata/plexe-pyapi
    def get_distance(self, v1, v2):
        ...
        if v1  not in veh_tlp and v2 not in veh_tlp:
            ...
            for vid in traci.simulation.getDepartedIDList():
                ...
                if traci.vehicle.getTypeID(vid) == "cav2":
                    ...
                    if traci.vehicle.getLeader(vid) is None:
                        ...
                        if vid_front in self.platoons:
                            leader_id = vid_front
                        elif vid_front in self.topology:
                            leader_id = self.topology[vid_front]["leader"]
                        else:
                            leader_id = None
                        if leader_id and self.platoons[leader_id]["ini_size"] <= len(self.platoons[leader_id]["members"]):
                            self.platoons[vid] = {"ini_size": random.randint(min_platoon_size, max_platoon_size),
                                                "members": [vid], "state": 0}
                        else:
    ...
    def update_teleport_vehicles(self):
        veh_tlp = traci.simulation.getCollidingVehiclesIDList()
        logging.debug(f"Colliding vehicles: {veh_tlp}")
        if len(veh_tlp) != 0:
            for veh in veh_tlp:
                logging.debug(f"Processing teleporting vehicle {veh}")
                if veh in self.platoons.keys():
                    leader_id = veh
                    p = self.platoons[leader_id]["members"]
                    for v in p:
                        logging.debug(f"Removing vehicle {v} from topology and simulation")
                    ...
                    logging.debug(f"Removing vehicle {veh} from simulation")
                    traci.vehicle.remove(veh)
                # Initialize new platoons for teleporting vehicles
                self.platoons[veh] = {"ini_size": random.randint(self.min_platoon_size, self.max_platoon_size),
                                  "members": [veh]}  # Assuming `members` should be initialized
    ...
    def do_leaves(self):
        ...
        for i in range(len(self.leave_list)):
            ...
            if state == self.CHANGING_LANE and traci.vehicle.getLaneIndex(leave_id) != str(int(traci.edge.getLaneNumber(traci.vehicle.getRoadID(leave_id))) - 1):
                ...
                # print("vehicle left the platoon:", self.leave_list[i])
    ...
    def wait_for_vehicles(self):
        # Wait until all vehicles are in the simulation
        while not all(vehicle_id in traci.vehicle.getIDList() for vehicle_id in self.vehicle_ids):
            traci.simulationStep()
            self.debug_vehicle_ids()
    ...
    def handle_collisions(self):
        # Handle collisions by stopping or slowing down vehicles instead of removing them
        for vehicle_id in traci.vehicle.getIDList():
            collisions = traci.simulation.getCollidingVehiclesIDList()
            if vehicle_id in collisions:
                traci.vehicle.setSpeed(vehicle_id, 0)  # Stop the vehicle
                logging.warning(f"Collision detected for vehicle {vehicle_id}. Stopping the vehicle.")
